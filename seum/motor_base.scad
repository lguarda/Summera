...
module motor_base(ears_dist, ears_diam, angle, h=4) {
    new_x = cos(angle) * ears_dist;
    new_y = -sin(angle) * ears_dist;
    if ($preview) {
        move([ ears_dist, 0 ])
            cylinder(h=h, d=ears_diam*4, center=true);
        move([ new_x, new_y ])
            cylinder(h=h, d=ears_diam*4, center=true);
        cylinder(h=h, d=19, center=true);
        ...
        linear_extrude(height = h, center = true, convexity = 1, slices = 1, $fn = 1)
